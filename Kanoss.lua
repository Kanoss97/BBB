...
function scsettings()
  xcs = gg.choice({
    'Hack tốc độ farm nến : ' .. psettings.crspeed,
    'Độ trễ của tự động farm nến : ' .. psettings.crdelay .. 'ms',
    'Tự động farm nến sau độ trễ : ' .. psettings.crabsorb .. 'ms',
    'Độ trễ của tự động farm cánh : ' .. psettings.wrdelay .. 'ms',
    'Breach wall distance : ' .. psettings.warpdis,
    'Không gây hại cho cánh : ' .. boolling(psettings.nodamage),
    'Về home nhanh : ' .. boolling(psettings.fasthome),
    'Sử dụng gg hack tốc độ : ' .. boolling(psettings.ggspeed),
    'Hack tốc độ khi thay đổi map : ' .. boolling(psettings.portalspeed),
    'Tự động hiện lên script menu : ' .. boolling(psettings.showmenu),
    'Tốc độ chạy của Semi-auto farm nến : ' .. psettings.smcrdelay .. 'ms',
    'Tốc độ chạy của Semi-auto nhặt cánh : ' .. psettings.smwrdelay .. 'ms',
    'Sử dụng trình thay đổi bản đồ cũ : ' .. boolling(psettings.portaldef),
    'Tốc độ về nhà : ' .. psettings.fhspeed,
    'Thay đổi hình ảnh map : ' .. imgs[psettings.cmimage],
    ...
    message = ''
      ...
      if m == 4 then 
        ...
        table.insert(y,'⚠️Crash game')
         r=gg.choice(y,nil,'Chọn map và sử dụng cánh của bạn! ')
         if (r ~= nil) then 
           gg.setVisible(false)
      ...
      if m == 5 then
         ...
         x=gg.choice({
           ...
           '🏂Chế độ trượt',
           '📳FPS',
           '🚸Kích cỡ thân hình(Chỉ bản thân thấy)',
           '🚹Không bị hạ gục',
           '🔥Tự động cháy',
           '🌬Loại bỏ bức tường gió',
           '🏠Về nhà nhanh\nến',
           '🔦Ánh sáng nhân lên',
           '🏜Làm sáng map',
           '🏃Honk to jump',
           '📥Hồi tức thì thời gian item',
           '🦿No movement block'
         },nil,'')
          if x == 1 then 
